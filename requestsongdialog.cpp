...
RequestSongDialog::RequestSongDialog(const sf::String& text, const sf::Vector2f background_size,
  ...
  m_authorInput = TextInput(textInputSize,m_font);
  ...
  m_songInput = TextInput(textInputSize,m_font);
...
void RequestSongDialog::symErase() {
  sf::String curText;

  TextInput* activeInput = getActiveInput();
  if (!activeInput) return;

  curText = activeInput->getText();

  if (curText.isEmpty()) return;
  activeInput->setText(curText.substring(0, curText.getSize()-1));
}
