...
Button::Button(sf::Vector2f size, sf::String text, sf::Font font, sf::Color bgColor, int textSize)
	: m_body(size), font(font), pressed(false), text(this->font)
{
	...
	//this->text.setPosition(this->m_body.getPosition().x - this->text.getGlobalBounds().size.x / 2, this->m_body.getPosition().y - this->m_body.getGlobalBounds().size.y / 8);
	this->text.setPosition(m_body.getPosition());
	this->text.setFont(this->font);
	this->text.setCharacterSize(textSize);

	this->m_body.setOrigin(sf::Vector2f(this->m_body.getGlobalBounds().size.x / 2, this->m_body.getGlobalBounds().size.y / 2));
	this->text.setOrigin(sf::Vector2f(this->text.getGlobalBounds().size.x / 2, (this->text.getGlobalBounds().size.y / 2) + this->text.getGlobalBounds().size.y / 4));
...
Button::Button(sf::Vector2f size, sf::String text, sf::Color bgColor, int textSize)
	: m_body(size), pressed(false), text(this->font)
{
	m_source_background_color = bgColor;
	m_source_text_color = sf::Color::White;
	...
	this->font.openFromFile(NOTOSANS_FONT_PATH);
